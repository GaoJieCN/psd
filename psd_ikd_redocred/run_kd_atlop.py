...
from re_evaluation import to_official, official_evaluate, official_evaluate_benchmark
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) == len(train_dataloader) - 1 or (args.evaluation_steps > 0 and num_steps % args.evaluation_steps == 0 and step % args.gradient_accumulation_steps == 0):
                    ...
                    if dev_score > best_score:
                        ...
                        dump_json({'dev_output': dev_output, 'test_f1': test_output}, _path)
...
def evaluate(args, model, features, tag):
    ...
    if tag == 'dev':
        # docred_metric = official_evaluate(ans, os.path.join(args.root_path, args.data_dir), args.train_file, args.dev_file)
        redocred_metric = official_evaluate_benchmark(ans, os.path.join(args.root_path, args.data_dir), args.train_file, args.dev_file)

    elif tag == 'test':
        redocred_metric = official_evaluate_benchmark(ans, os.path.join(args.root_path, args.data_dir), args.train_file, args.test_file)


    output = {tag: redocred_metric}
    return redocred_metric['re_f1'], output
...
def main():
    ...
    if args.mode == 'test':  # Testing
        ...
        dev_f1, dev_output = evaluate(args, model, dev_features, tag="dev")
        test_f1, test_output = evaluate(args, model, test_features, tag='test')
        print(dev_output)
